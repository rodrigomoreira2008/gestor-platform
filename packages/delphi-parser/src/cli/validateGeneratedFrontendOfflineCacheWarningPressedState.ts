...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningPressedStateGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningPressedState.ts <dfm> <pas> <entidade> <tabela>');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes('setOfflineCacheWarningDismissed')
];

const passed = checks.filter(Boolean).length;
if (passed !== checks.length) {
  console.error(`FRONTEND_OFFLINE_CACHE_WARNING_PRESSED_STATE_ERROR:${entity}:checks=${checks.length}:passed=${passed}`);
  process.exit(1);
}
console.log(`FRONTEND_OFFLINE_CACHE_WARNING_PRESSED_STATE_OK:${entity}:checks=${checks.length}:passed=${passed}`);
