...
import { generateFrontendFiles } from '../frontendOfflineRefreshGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2).filter((arg) => arg !== '--json');
const json = process.argv.includes('--json');

if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineRefresh.ts <dfm> <pas> <entidade> <tabela> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  ['refresh disabled while paused', source.includes("disabled={list.isFetching || list.fetchStatus === 'paused'}")],
  ['offline button label', source.includes("list.fetchStatus === 'paused' ? 'Sem conexão'" )],
  ['fetching label preserved', source.includes("list.isFetching ? 'Atualizando...' : 'Atualizar'" )],
  ['manual refetch preserved', source.includes('onClick={() => void list.refetch()}')],
...
else if (report.ok) console.log(`FRONTEND_OFFLINE_REFRESH_OK:${entity}:checks=${report.checks}:passed=${report.passed}`);
else console.error(`FRONTEND_OFFLINE_REFRESH_ERROR:${entity}:failed=${report.failed.join(',')}`);
