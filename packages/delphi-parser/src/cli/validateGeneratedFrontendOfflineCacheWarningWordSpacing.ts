...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningWordSpacingGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningWordSpacing.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  page.content.includes("wordSpacing: 'inherit'"),
  page.content.includes("letterSpacing: 'inherit'"),
  page.content.includes("lineHeight: 'inherit'"),
  page.content.includes("fontSize: 'inherit'"),
  page.content.includes("fontFamily: 'inherit'"),
  page.content.includes("fontWeight: 'normal'"),
  page.content.includes("fontStyle: 'normal'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_WORD_SPACING_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
