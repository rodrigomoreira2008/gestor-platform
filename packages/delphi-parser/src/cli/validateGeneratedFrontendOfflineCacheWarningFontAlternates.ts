...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningFontAlternatesGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningFontAlternates.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes("fontVariantCaps: 'normal'"),
  page.content.includes("fontVariantNumeric: 'normal'"),
  page.content.includes("fontVariantLigatures: 'normal'"),
  page.content.includes("fontKerning: 'normal'"),
  page.content.includes("hangingPunctuation: 'none'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_FONT_ALTERNATES_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
