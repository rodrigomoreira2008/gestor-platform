...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningFontNumericGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningFontNumeric.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes("textAlign: 'start'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_FONT_NUMERIC_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
