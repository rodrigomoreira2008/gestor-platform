...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningTouchTargetGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningTouchTarget.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes('minHeight: 44'),
  page.content.includes("justifyContent: 'center'"),
  page.content.includes("touchAction: 'manipulation'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_TOUCH_TARGET_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
