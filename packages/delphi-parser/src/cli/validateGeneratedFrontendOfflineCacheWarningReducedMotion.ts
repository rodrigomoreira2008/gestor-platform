...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningReducedMotionGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningReducedMotion.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes("transition: 'none'"),
  page.content.includes("transform: 'none'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_REDUCED_MOTION_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
