...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningLiveRelevantGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningLiveRelevant.ts <dfm> <pas> <entidade> <tabela>');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes('aria-relevant="text"'),
...
if (passed !== checks.length) {
  console.error(`FRONTEND_OFFLINE_CACHE_WARNING_LIVE_RELEVANT_ERROR:${entity}:checks=${checks.length}:passed=${passed}`);
  process.exit(1);
}
console.log(`FRONTEND_OFFLINE_CACHE_WARNING_LIVE_RELEVANT_OK:${entity}:checks=${checks.length}:passed=${passed}`);
