...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningLiveStateGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningLiveState.ts <dfm> <pas> <entidade> <tabela>');
  process.exit(1);
}

const resolved = resolveForm(
  parseDfm(readFileSync(resolve(dfmPath), 'utf8')),
  parsePas(readFileSync(resolve(pasPath), 'utf8')),
  { entity, table }
);
...
const checks = [
  page.content.includes('aria-live="polite"'),
  page.content.includes('aria-atomic="true"'),
  ...
  page.content.includes('offlineCacheWarningChipRef'),
...
if (passed !== checks.length) {
  console.error(`FRONTEND_OFFLINE_CACHE_WARNING_LIVE_STATE_ERROR:${entity}:checks=${checks.length}:passed=${passed}`);
  process.exit(1);
}
console.log(`FRONTEND_OFFLINE_CACHE_WARNING_LIVE_STATE_OK:${entity}:checks=${checks.length}:passed=${passed}`);
