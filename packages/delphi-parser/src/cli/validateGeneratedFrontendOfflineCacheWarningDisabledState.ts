...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningDisabledStateGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningDisabledState.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_DISABLED_STATE_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
