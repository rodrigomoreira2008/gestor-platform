...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningTextReflowGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningTextReflow.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes("'& .MuiChip-label'"),
  page.content.includes("display: 'block'"),
  page.content.includes("overflowWrap: 'anywhere'"),
  page.content.includes("textAlign: 'center'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_TEXT_REFLOW_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
