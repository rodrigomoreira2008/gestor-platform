...
if (existsSync(workflowPath)) {
  ...
  const requiredFragments = [
    ...
    'validate:fixture-frontend-runtime', 'validate:fixture-routes', 'validate:fixture-semantic', 'validate:fixture-coverage'
  ];
  ...
  const requiredCommands = [
    ...
    'validate:fixture-backend-runtime', 'validate:fixture-frontend-compile', 'validate:fixture-frontend-runtime',
  ...
  checks.push({
    ...
    ok: artifactStep >= 0 && pathsStep > artifactStep && placeholdersStep > pathsStep && budgetsStep > placeholdersStep && textFormatStep > budgetsStep && namesStep > textFormatStep && boundariesStep > namesStep && syntaxStep > boundariesStep && importsStep > syntaxStep && determinismStep > importsStep && contractsStep > determinismStep && repositoryStep > contractsStep && backendCompileStep > repositoryStep && backendRuntimeStep > backendCompileStep && frontendCompileStep > backendRuntimeStep && frontendRuntimeStep > frontendCompileStep && routesStep > frontendRuntimeStep && semanticStep > routesStep && coverageStep > semanticStep,
    detail: `artefatos=${artifactStep}, caminhos=${pathsStep}, placeholders=${placeholdersStep}, orcamentos=${budgetsStep}, formato=${textFormatStep}, nomes=${namesStep}, fronteiras=${boundariesStep}, sintaxe=${syntaxStep}, imports=${importsStep}, determinismo=${determinismStep}, contratos=${contractsStep}, persistencia=${repositoryStep}, backend=${backendCompileStep}, runtime-backend=${backendRuntimeStep}, frontend=${frontendCompileStep}, runtime-frontend=${frontendRuntimeStep}, rotas=${routesStep}, semantica=${semanticStep}, cobertura=${coverageStep}`
  });
}
