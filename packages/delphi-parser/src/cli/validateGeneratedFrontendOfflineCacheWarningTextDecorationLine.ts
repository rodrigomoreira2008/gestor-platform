...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningTextDecorationLineGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningTextDecorationLine.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  page.content.includes("textDecorationLine: 'none'"),
  page.content.includes("textDecorationColor: 'currentColor'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_TEXT_DECORATION_LINE_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
