...
import { generateFrontendFiles } from '../frontendOfflineCacheWarningFocusVisibleGenerator';

const [dfmPath, pasPath, entity, table] = process.argv.slice(2);
if (!dfmPath || !pasPath || !entity || !table) {
  console.error('Uso: tsx validateGeneratedFrontendOfflineCacheWarningFocusVisible.ts <dfm> <pas> <entity> <table> [--json]');
  process.exit(1);
}
...
const checks = [
  ...
  page.content.includes("outline: '3px solid'"),
  page.content.includes("outlineColor: 'primary.main'"),
  page.content.includes("outlineOffset: '2px'"),
...
else console.log(`FRONTEND_OFFLINE_CACHE_WARNING_FOCUS_VISIBLE_${result.ok ? 'OK' : 'ERROR'}:${entity}:checks=${checks.length}:passed=${passed}`);
