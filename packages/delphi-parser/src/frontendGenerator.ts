...
function generatePage(entityPascal: string, entity: string, plural: string, detailGrids: InferredDetailGrid[]): string {
  ...
  const detailSections = detailGrids.map((grid) => `        <${entityPascal}${toPascalCase(grid.name)}DetailGrid rows={${toCamelCase(grid.name)}Details.data} isLoading={${toCamelCase(grid.name)}Details.isLoading} />`).join('\n');
